...
export const detect = async (page: Page, browser: Browser, resources: Resources) => {
  const { result, getAllResultsWithConfidence } = await calculateResults(resources, page, browser, patterns);

  const frameworkFeatures = new Set<typeof detectableFrameworkFeatures[number]>();

  detectableFrameworkFeatures
    .filter((feature): feature is typeof detectableFrameworkFeatures[number] => result.matched.has(feature))
