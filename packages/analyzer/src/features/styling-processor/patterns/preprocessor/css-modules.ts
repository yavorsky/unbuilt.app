...
export const cssModules = [
  {
    ...
    browser: async (page: Page) => {
      return page.evaluate(() => {
        ...
        const hasModuleClasses = Array.from(
          document.querySelectorAll('*')
        ).some((el) =>
          Array.from(el.classList).some((className) =>
            modulePattern.test(className)
          )
        );

        // Check stylesheets for CSS Modules patterns
        const hasModuleStyles = Array.from(document.styleSheets).some(
          (sheet) => {
            try {
              ...
              return rules.some((rule) => {
                if (rule instanceof CSSStyleRule) {
                  return (
                    modulePattern.test(rule.selectorText) ||
