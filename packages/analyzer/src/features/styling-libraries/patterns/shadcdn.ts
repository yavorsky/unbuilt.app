    ...
    browser: async (page: Page) => {
      return page.evaluate(() => {
        const markers = {
          ...
          hasRadixAttributes: !!document.querySelector('[data-radix-popper-content-wrapper]') ||
                            !!document.querySelector('[role="dialog"][data-state]'),

          // Check for common shadcn class patterns
          hasShadcnClasses: document.querySelector('[class*="scroll-area"], [class*="select-none"]') !== null,

          // Check for Radix state attributes
          hasStateAttributes: document.querySelector('[data-state="open"], [data-state="closed"]') !== null,
