...
export const MinifierCard: FC<{
  minifier: AnalyzeResult['analysis']['minifier'];
}> = ({ minifier }) => {
  const [isOpen, setIsOpen] = useState(false);
  const hasAlternatives = Object.keys(minifier.secondaryMatches).length > 0;
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Badge>
                <span className="text-gray-300 text-xl">{minifier.name}</span>
              </Badge>
              <div className="flex items-center gap-2">
                <ConfidenceIndicator confidence={minifier.confidence} />
              </div>
            </div>
            ...
            {hasAlternatives && (
              <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                <CollapsibleTrigger className="flex items-center gap-2 text-sm text-gray-400 hover:text-gray-300 transition-colors">
                  ...
                  {Object.keys(minifier.secondaryMatches).length} alternative
                  bundlers
                </CollapsibleTrigger>
                <CollapsibleContent className="pl-4 space-y-1 mt-2">
                  {Object.entries(minifier.secondaryMatches)
