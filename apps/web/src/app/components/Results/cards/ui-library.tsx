...
export const UILibraryCard: FC<{
  uiLibrary: AnalyzeResult['analysis']['uiLibrary'];
}> = ({ uiLibrary }) => {
  const [isOpen, setIsOpen] = useState(false);
  const hasSecondaryMatches =
    Object.keys(uiLibrary.secondaryMatches).length > 0;
    ...
    <Card className="bg-gray-900 border-gray-800 hover:border-indigo-500 transition-all duration-300">
      <CardContent className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-100">UI Library</h3>
        </div>
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Badge>
                <span className="text-gray-300 text-xl">{uiLibrary.name}</span>
              </Badge>
              <div className="flex items-center gap-2">
                <ConfidenceIndicator confidence={uiLibrary.confidence} />
              </div>
            </div>
            ...
            {hasSecondaryMatches && (
              <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                <CollapsibleTrigger className="flex items-center gap-2 text-sm text-gray-400 hover:text-gray-300 transition-colors">
                  ...
                  {Object.keys(uiLibrary.secondaryMatches).length} secondary
                  matches
                </CollapsibleTrigger>
                <CollapsibleContent className="pl-4 space-y-1 mt-2">
                  {Object.entries(uiLibrary.secondaryMatches)
