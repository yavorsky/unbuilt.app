...
export const FrameworkCard: FC<{
  framework: AnalyzeResult['analysis']['framework'];
}> = ({ framework }) => {
  const [isOpen, setIsOpen] = useState(false);
  const hasAlternatives = Object.keys(framework.secondaryMatches).length > 0;
    ...
    <Card className="bg-gray-900 border-gray-800 hover:border-indigo-500 transition-all duration-300">
      <CardContent className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-100">Framework</h3>
        </div>
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <Badge>
                <span className="text-gray-300 text-xl">{framework.name}</span>
              </Badge>
            ...
            {hasAlternatives && (
              <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                <CollapsibleTrigger className="flex items-center gap-2 text-sm text-gray-400 hover:text-gray-300 transition-colors">
                  ...
                  {Object.keys(framework.secondaryMatches).length} alternative
                  frameworks
                </CollapsibleTrigger>
                <CollapsibleContent className="pl-4 space-y-1 mt-2">
                  {Object.entries(framework.secondaryMatches)
